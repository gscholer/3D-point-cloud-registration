...
To elaborate on the model let us first define $\mathbf{X}=[X,Y,Z]^T$ as the coordinates of some 3D world point and $\mathbf{x}=[x,y]^T$
...
To project $\mathbf{X}$ onto the image plane we first need another point in the 3D world called the optical center, $\mathbf{O}$.
...
$$y=f\frac{X}{Z} \quad,\quad x=f\frac{Y}{Z}\quad \quad (1)$$
...
1. Randomly sample n points and instantiate the model from where we obtain the model fitted parameters, $\widehat{\Theta}$.
2. For all points compute the error $\varepsilon _i = |Y_i-\widehat{\Theta }X_i|$
...
the model is given by $P=1-(1-p^n)^k$, then, to estimate the number of iterations to use, we can use:
...
$$M=p_{0}\,q_{0}^T=U\Sigma V^T \quad \quad (4)$$

$$\widehat{R}=VU^T \quad \quad (5)$$

$$\widehat{\mathbf{T}}=\sum_{i=1}^{N} \frac{\mathbf{p_i}}{N}-\widehat{R}\sum_{i=1}^{N} \frac{\mathbf{q_i}}{N}$$

Where $U\Sigma V^T$ is the singular value decomposition of M. A detailed proof as to why these $\widehat{R}$
and $\widehat{\mathbf{T}}$ are the ones that minimize Equation 4 can be found in Appendix A.
...
$i^{th}$ RGB image $RGB_i$ and its corresponding depth image $D_i$.

The first sub-problem we need to address in this project is the combination of the data present in an RGB/depth image pair to include on the 3D scene. We can define the image coordinates *u* and *v* to simply be the indices of each pixel (with (0,0) coinciding with the top left corner), and Z to be the corresponding depth at those coordinates. We can then use Equation 3 with R “ I (identity matrix) and $T=[0,0,0]^T$ (since we are still in the depth camera reference frame, and thus there is no geometric transformation of the coordinates apart from the pixel to meter conversion) to compute the X and Y coordinates. The corresponding equation is the following one, which is simply an inversion of Equation 3 with the specified *R* and $\mathbf{T}$.
